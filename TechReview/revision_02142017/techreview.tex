...
We did this for the exact reasons I mentioned above, it has allowed us to only parse what we needed instead of using a third-party parser which might parse for a ton of stuff.
...
This was also helpful because we didn't have to go to an outside tool to do the parsing.
For example if we had used Perl, we would have had to call our Perl script from JavaScript. 
...
This also allows the development team to use NodeJS and its many useful libraries. 
...
While not in use yet, the plan is to use the Jquerey advanced newsticker for displaying the list of errors.
