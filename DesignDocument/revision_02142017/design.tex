...
\subsection{Design Elements}
	...
	Type: system
	Description: The design of the extension is meant to help new web developers make better design decisions when writing HTML, CSS, PHP and JavaScript code.
...
\subsection{Design Elements}
	...
	\subsection{Files}
		...
		\begin{itemize}
			\item Load Data
			Once the information about the file nodes from the data structure are pulled from the JSON file, it will be updated in the project visualization UI to identify which links or errors have changed. 
			\item Save Data
			Once parsed data from the Parser is transferred to the data structure using the Get Data functionality, this data will be serialized into a JSON file with the JSON.stringify function built into JavaScript. \cite{stringify}
		\end{itemize}
		\subsubsection{Requires}
		\begin{itemize}
			\item Get Data (data structure)
