...
export function activate(context: vscode.ExtensionContext) {
    ...
    let parse = vscode.commands.registerCommand('extension.parse', () => {
        ...
        //Get All Directory Paths and token info
    //    function getDirTokenInfo(){
    //         var dirsInfo = [];
    //         var topdirPaths = nodefs.readdirSync(vscode.workspace.rootPath);
    //         var dirPaths = find.dirSync(vscode.workspace.rootPath);
    //         for(var x = 0; x < dirPaths.length; x++){
    //             dirsInfo.push({
    //                 type: "directory",
    //                 path: dirPaths[x],
    //                 name: dirPaths[x].slice(dirPaths[x].lastIndexOf("\\")+1),
    //             })
    //         }
    //        // console.log(JSON.stringify(dirPaths));
    //         //console.log(JSON.stringify(topdirPaths));

    //      //   return dirsInfo;
    //     }
        
    //     //Send File Paths to Parser and get token info
    //     function getFileTokenInfo(){
    //         var filesInfo = []; //Tokens from parser.ts
    //         var filePaths = find.fileSync(vscode.workspace.rootPath);
    //         for(var x = 0; x < filePaths.length; x++){
    //             //filesInfo.push(parse(filePaths[x]));
    //         }
    ...
    //     function getLevel() {
    //         //Backslashes in working directory
    //         var topSlashCounter = (vscode.workspace.rootPath.match(/\//g) || []).length;
    ...
    //     }
        ...
        /*function setFileData() {
            ...
            var dirsInfo = getDirTokenInfo();

            dirsInfo.forEach(function(index){
                FileStructs.push({
                            id: index,
                            level: ,
                            isSubContainer: , //bool, Not files or dirs
                            name: ,
                            type: ,
                            path: ,
                            links: [],
